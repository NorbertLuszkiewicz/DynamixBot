...
const getChessUser = async (name, streamer) => {
  try {
    ...
    const bulletData = userStats.chess_bullet
      ? `| BULLET: ${userStats.chess_bullet.last.rating}`
      : "";
    const blitzData = userStats.chess_blitz
      ? `| BLITZ: ${userStats.chess_blitz.last.rating}`
      : "";
    const rapidData = userStats.chess_rapid
      ? `| RAPID: ${userStats.chess_rapid.last.rating}`
      : "";
    const tacticsData = userStats.chess_bullet
      ? `| ZADANIA: najwyżej ${userStats.tactics.highest.rating}`
      : "";
    const bestRapidGame =
...
const getLastGame = async (name, streamer) => {
  try {
    const now = new Date();

    let {body: {games}} = await chessAPI.getPlayerCompleteMonthlyArchives(
      name,
      now.getFullYear(),
      now.getMonth() + 1
    );
    ...
    if(games.length === 0 && now.getMonth() !== 0){
      games = await chessAPI.getPlayerCompleteMonthlyArchives(
          name,
          now.getFullYear(),
          now.getMonth()
        )
